// Weather Map with Airline Data; 2018.02.05 Mooyoung Lee, Yao Yao, Sunna Quazi //<>//
// Reference: 
...
void setup() {
  ...
  // SNOW //
  ps = new ParticleSystem(new PVector(width*.45, 0));
  
  //PLANE//
  aerop= loadImage("plane2.png");
  for(int i=0; i<aero.length; i++){
  aero[i]= new airplane(0, 500, 100);
  }
  //ball
    //noFill();
...
// SNOW//
  ...
  //ball
  stroke(255, 201, 14, 170);
  fill(255, 0, 0, 200);
    for (int i = 0; i < p.length-1; i++) line(p[i].x, p[i].y, p[i+1].x, p[i+1].y);
