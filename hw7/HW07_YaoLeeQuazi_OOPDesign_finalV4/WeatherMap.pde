class WeatherMap {
  ...
  void display() {
    ...
    for (int j = 0; j < airportLoc.length; j++) {
      ...
      airY = int(airportLoc[j][1]*width2/imgLength+imgY/2)-20;
      ...
      if (distMouse2 < mouseRadius) {
        ...
        // Plane Path
        int noCancel = int(cancel[j][2]/100*airportLoc.length);
        int noDelay = int(noCancel+cancel[j][1]/100*airportLoc.length);
        println(noCancel);
        for (int k = 0; k < airportLoc.length; k++) {
          if (k < noCancel) {
            stroke(255, 0, 0);
          } else if (k >= noCancel && k <noDelay) {
            stroke(255, 128, 0);
          } else {
            stroke(0, 255, 0);
          }
          if (k != j) {
            line(airX+35, airY+20, int(airportLoc[k][0]*width2/imgLength+35), int(airportLoc[k][1]*width2/imgLength+20));
            //noStroke();
          }
        }
