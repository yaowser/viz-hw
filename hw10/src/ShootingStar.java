...
public class ShootingStar {
  ...
  private float[] z = new float[particleCount];
  private float[] z2 = new float[particleCount];
  ...
  private float[] speedZ= new float[particleCount];
  ...
  public ShootingStar(PApplet p) {
    ...
    for (int i=0; i< particleCount; i++) {
      ...
      z2[i] = p.random(zmin+100, zmax-100);
    }
  }
  ...
  public void display(PApplet p) {
    ...
    for (int i=0; i< particleCountCurrent; i++) { 
      ...
      p.line(x[i], y[i], z[i], x[i]+speedX[i], y[i]+speedY[i]*2, z[i]+speedZ[i]*2) ;
