...
AudioPlayer song;
AudioMetaData meta;
String[] cool = {"Hans Zimmer - Leaving Wallbrook - On the Road.mp3","Beethoven - 7th Symphony - 2nd movement.mp3","Isaac Hayes - Hung Up On My Baby.mp3","Plácido Domingo, Luciano Pavarotti & José Carreras - La Traviata.mp3","Keith Mansfield - Love de Luxe.mp3","Visioneers - Ike's Mood I.mp3","Andrea Bocelli - Con Te Partiro.mp3"};
// SphereBands;
ArrayList<SphereBands> sbs = new ArrayList(1);
PImage sprite;
ParticleSystem ps; // Mouse follow particles
BlockBands bbs;
int binSize;
...
void playThis(String cool) {
  ...
  for ( int i = 0; i<fft.avgSize(); i++) {
    sbs.add(new SphereBands());
  }
  bbs = new BlockBands(fft.avgSize());
}
...
void draw() {
  ...
  /*This mixes the left and right channels of the audio input and performs the fft*/
  ...
  for (int i = 0; i<binSize; i++) { 
    ...
    if (s == 1) {
      ...
      sbs.get(i).display(i*height/100+100, amplitude/5,  p);
    } else {
      //display(float amplitude, float yRotate, int binSize, int i)
      bbs.display(amplitude*10, q, binSize, i);
    }
  }
  ps.update();
  ps.display();
